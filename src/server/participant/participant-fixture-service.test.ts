...
describe("listParticipantFixtures security", () => {
  ...
  it("returns empty lists when account has no selected memberships", async () => {
    ...
    const bundle = await listParticipantFixtureBundle({ accountId: "account-1" });
    expect(bundle.all.upcoming).toEqual([]);
    expect(bundle.all.completed).toEqual([]);
    expect(bundle.mine.upcoming).toEqual([]);
    expect(bundle.mine.completed).toEqual([]);

    const mine = await listParticipantFixtures({
      accountId: "account-1",
      scope: "mine",
    });
    expect(mine.upcoming).toEqual([]);
    expect(mine.completed).toEqual([]);
  });
});
